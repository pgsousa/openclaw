...
function buildFixContinuationDirective(alertId: string): string {
  return [
    ...
    "Do not invent approval ids or action ids.",
    "Assume exec tool is available; if exec fails, treat that as a blocker and quote the exact error.",
    "For read-only diagnostics: run the commands directly via the exec tool (no approval).",
    "For mutating actions: call the exec tool to create a runtime exec approval request, then ask the operator to approve that runtime id: approve <id> [allow-once|allow-always|deny].",
    "The operator may approve using the full id or the unique 8-character id prefix.",
    "Never ask the operator to run kubectl manually unless exec tool execution failed and you can quote the exact failure.",
    'Do not impose artificial command-count limits (no "exactly 3" / "max 2" rules).',
